...
export default function ComparePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api<Record<string, unknown>>("/api/v1/portfolio/latest").catch(() => null),
      api<Record<string, unknown>>("/api/v1/risk/latest").catch(() => null),
    ]).then(([portfolio, risk]) => {
      if (portfolio) {
        const allocs: Record<string, number> = {};
        const rawAllocs = (portfolio.allocations as Array<{ asset_class: string; allocation_pct: number }>) || [];
        for (const a of rawAllocs) {
          allocs[a.asset_class] = (a.allocation_pct as number) / 100;
        }
        setCurrentPortfolio({
          risk_score: (risk?.overall_score as number) ?? 0,
          risk_category: (risk?.risk_category as string) ?? "moderate",
          allocations: allocs,
          expected_return_range: {
            low: portfolio.expected_return_min as number,
            high: portfolio.expected_return_max as number,
          },
        });
      }
    }).catch((err) =>
  ...
  const generateAlternative = async () => {
    ...
    try {
      ...
      const toAllocMap = (p: Record<string, unknown>): Record<string, number> => {
        const allocs: Record<string, number> = {};
        const portfolio = p as { allocations?: Array<{ asset_class: string; allocation_pct: number }> };
      ...
      setAlternative({
        ...
        score_change: (mod.risk_score as number) - (base.risk_score as number),
